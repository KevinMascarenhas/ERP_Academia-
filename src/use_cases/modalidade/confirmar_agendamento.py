...
class ConfirmarAgendamentoUseCase:
    # Confirma um agendamento de aula.
    ...
    def executar(self, idx_aluno, idx_inscricao):
        ok = self.aluno_repo.confirmar_agendamento(idx_aluno, idx_inscricao)
        if ok:
            ...
            self._logger.registrar(aluno_nome, f"Confirmação de agendamento: {modalidade_nome} em {data_hora}")
