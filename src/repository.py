...
def adicionar_usuario(usuario: Usuario):    # para que usuarios seja uma lista de objetos da classe Usuario, defini o type hint "usuario: Usuario". O mesmo ocorre com as outras funções abaixo.
...
def adicionar_administrador(admin: Administrador):    # para que usuarios seja uma lista de objetos da classe Usuario, defini o type hint "usuario: Usuario". O mesmo ocorre com as outras funções abaixo.
...
def inicializar_dados() -> None:
    ...
    adicionar_plano(Plano("Basic",  59.90,  1, 1))
    adicionar_plano(Plano("Mensal", 99.90,  3, 1))
    adicionar_plano(Plano("Anual",  799.90, 999, 12))
 
    adicionar_modalidade(Modalidade("Musculação", "Força",       "06:00 - 22:00"))
    adicionar_modalidade(Modalidade("Natação",    "Aquática",    "07:00 - 09:00"))
    adicionar_modalidade(Modalidade("Yoga",       "Relaxamento", "08:00 - 09:00"))
    adicionar_modalidade(Modalidade("Muay Thai",  "Luta",        "19:00 - 20:30"))
 
    plano_mensal = buscar_plano_por_nome("Mensal")
    joao = Aluno("João Silva", "joao@email.com", "1234", "111.111.111-11", plano_mensal)
    joao.frequentar(listar_modalidades()[0])
